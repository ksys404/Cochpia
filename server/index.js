...
async function handleChatStream(req, res, { regenerateMessageId = null, retry = false } = {}) {
  ...
  if (state.mode === 'work') {
    ...
    try {
      ...
      const rt = buildRuntimeContext({ messages: [], personality: state.personality, recalled, memoryBundle, summary, persona: session.persona, upcomingEvents: events.listUpcoming(7), atmosphere: resolveAtmosphere(session.atmosphere)?.tone, profile: state.profile, mode: state.mode });
  ...
  try {
    for await (const delta of selectedModel.stream({
      ...
      recalled,
      runtimeContext: buildRuntimeContext({ messages: state.messages[sessionId], personality: state.personality, recalled, memoryBundle, summary, persona: session.persona, upcomingEvents: events.listUpcoming(7), atmosphere: resolveAtmosphere(session.atmosphere)?.tone, profile: state.profile, mode: state.mode }),
