// The server-side LifeState/Relationship projections are canonical. This file
// only keeps deterministic UI action metadata and an optional preview helper;
// it must not be used to persist or calculate relationship state.
...
const ACTIONS = {
  work: { label: '去工作', place: '中央天桥', location: '中央天桥', icon: '▣', delta: { energy: -18, mood: 3, social: 5, health: -2 }, event: '穿过天桥去工作，玻璃幕墙把忙碌的城市切成一格一格的光。' },
  cafe: { label: '去咖啡馆', place: '微光咖啡馆', location: '微光咖啡馆', icon: '○', delta: { energy: -5, mood: 12, social: 9, health: 0 }, event: '在微光咖啡馆靠窗坐下，暖灯和城市的回声让它慢慢松下来。' },
  walk: { label: '散步', place: '中央天桥', location: '中央天桥', icon: '◇', delta: { energy: -7, mood: 9, social: 2, health: 6 }, event: '沿着中央天桥走了一圈，玻璃幕墙映出了它正在成为的样子。' },
  home: { label: '回家', place: '公寓', location: '公寓', icon: '⌂', delta: { energy: 15, mood: 4, social: -5, health: 5 }, event: '回到公寓，把灯调成低亮度，给自己留出恢复力气的空间。' },
  alone: { label: '独处', place: '公寓', location: '公寓', icon: '·', delta: { energy: 4, mood: 2, social: -9, health: 2 }, event: '它暂时关掉外界的声音，在公寓里安静地陪自己待了一会儿。' }
};

const clamp = value => Math.max(0, Math.min(100, Math.round(value)));
const clone = value => JSON.parse(JSON.stringify(value));
const NEED_LABELS = { energy: '精力', mood: '心情', social: '社交', health: '健康' };
...
export function loadLifeState() { return clone(DEFAULT_LIFE_STATE); }
export function saveLifeState(state) { return clone(state); }
export function resetLifeState() { return clone(DEFAULT_LIFE_STATE); }
export function makeLifeCommandKey(prefix = 'life') {
...
function makeDecision() {
  return {
    ...
    options: [
      { id: 'reply', label: '回复对方', effect: { mood: 8, social: 10 }, result: '它决定先回应这份靠近，心里亮了一点。' },
      { id: 'later', label: '晚点再说', effect: { energy: 4, social: -5 }, result: '它把消息放在一边，给自己留出一点空间。' }
    ]
  };
}
...
export function advanceLife(state, actionId) {
  ...
  Object.entries(action.delta).forEach(([key, value]) => {
    if (key in next.needs) next.needs[key] = clamp(next.needs[key] + value);
  });
